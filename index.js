 ...
 *
 * FIXES:
 * - Corrected | vs || bug for env variables.
 * - Added static serving for /.well-known/ai-plugin.json and openapi.json
 * - Removed all /sse and MCP-related code to focus on the standard OpenAPI connection.
 */
...
//================================================================//
// ===== **NEW** ChatGPT Plugin Static Files ======================
// This serves your manifest and OpenAPI spec from the correct folder
//================================================================//
...
- "available": The restaurant confirmed availability for the requested time.
- "alternative_offered": The restaurant is booked at that time but offered a different time.
- "booked": The restaurant is fully booked and offered no alternatives.
- "credit_card_required": The restaurant has availability (original or alternative), but *explicitly* requires a credit card to hold the table.
- "left_message": We were sent to voicemail and left a message.
- "no_reservation_line": The call did not reach a human or reservation system (e.g., fax, disconnected).
- "opt_out": The human explicitly asked to be "taken off the list" or "do not call again".
- "other": The call was inconclusive, a human answered but the request was not understood, or an error occurred.

"ai_summary" rules:
- A *brief* one-sentence summary of what the human said.

"credit_card_required" rules:
- Set to true ONLY if the "outcome" is "available", "alternative_offered", or "credit_card_required" AND the human explicitly mentioned needing a credit card.
- If the outcome is "credit_card_required", this MUST be true.
  ...
  try {
    ...
    if (eventType === 'call.initiated') {
      ...
      if (result === 'machine_greeting_ended' || result === 'silence_detected' || result === 'beep_detected') {
        ...
        const msg = `Hi, this is a message about a reservation request for ${party_size} on ${date} at ${time}. Please call us back at ${TELNYX_PHONE_NUMBER.replace("+1", "")} if you have availability. Thank you.`;
...
// Use the new server start logic to set timeouts
const PORT = process.env.PORT || 3000;
const server = app.listen(PORT, () => {
    console.log(`✅ Rezkyoo server running at http://localhost:${PORT}`);
    
    // ** LOGGING FIX: NGROK_URL now contains the full URL **
    if (NGROK_URL) {
        console.log('---');
        // We log the *expected* paths, which ChatGPT will look for
